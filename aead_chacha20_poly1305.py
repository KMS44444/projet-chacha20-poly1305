...
from poly1305 import poly1305_mac, poly1305_key_gen_chacha20


def pad16(data:bytes) -> bytes:
    """
    Fonction rajoutant du padding (zéros) jusqu'à un multiple de 16 octets.
    Si la taille est déjà un multiple de 16 octets, alors aucun padding n'est ajouté.
    :param data: Tableau d'octets à aligner
    :return: Padding nécessaire sous forme de bytes nuls (b"\x00").
    """
    longueur = len(data)
    padding = (16 - (longueur % 16)) % 16
...
def aead_chacha20_poly1305_encrypt(key: int, nonce: int, plaintext: bytes, aad: bytes) -> Tuple[bytes, bytes]:
    ...
    """
    poly1305_key = poly1305_key_gen_chacha20(key, nonce)
    ...
    tag = poly1305_mac(bytes(mac_data), poly1305_key)
