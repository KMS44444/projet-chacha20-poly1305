...
def circular_shift_left(data: int, input_size: int, n_bit: int) -> int:
    ...
    """
    masque = (1 << input_size) - 1
    return ((data << n_bit) | (data >> (input_size - n_bit))) & masque
...
def quarter_round_on_selected(state: List[int], indices: List[int]) -> List[int]:
    ...
    """
    etat = list(state)
    ind_a, ind_b, ind_c, ind_d = indices
    ...
    res_a, res_b, res_c, res_d = quarter_round(val_a, val_b, val_c, val_d)
