...
def int_to_words(int_value: int, length: int) -> List[int]:
    ...
    masque = (1 << length) - 1
    safe_value = int_value & masque

    byte_array = safe_value.to_bytes(length // 8, 'big')
...
def state_builder(key: int, counter: int, nonce: int) -> List[int]:
    ...
    mots_cle = int_to_words(key, 256)
    counter_words = [counter]
    nonce_words = int_to_words(nonce, 96)

    return constantes + mots_cle + counter_words + nonce_words
...
def column_rounds(state: List[int]) -> List[int]:
    ...
    """
    etat = list(state)
    etat = quarter_round_on_selected(etat, [0, 4, 8, 12])
    etat = quarter_round_on_selected(etat, [1, 5, 9, 13])
    etat = quarter_round_on_selected(etat, [2, 6, 10, 14])
    etat = quarter_round_on_selected(etat, [3, 7, 11, 15])
    return etat
...
def diagonal_rounds(state: List[int]) -> List[int]:
    ...
    """
    etat = list(state)
    etat = quarter_round_on_selected(etat, [0, 5, 10, 15])
    etat = quarter_round_on_selected(etat, [1, 6, 11, 12])
    etat = quarter_round_on_selected(etat, [2, 7, 8, 13])
    etat = quarter_round_on_selected(etat, [3, 4, 9, 14])
    return etat
...
def twenty_rounds(state: List[int]) -> List[int]:
    ...
    """
    etat_temp = list(state)
    for _ in range(10):
        etat_temp = column_rounds(etat_temp)
        etat_temp = diagonal_rounds(etat_temp)
    return etat_temp
...
def add_lists_mod32(state1: List[int], state2: List[int]) -> List[int]:
    ...
    resultat = []
    for i in range(16):
        r = sum_mod_232(state1[i], state2[i])
        resultat.append(r)
    return resultat
...
def block_function(state: List[int]) -> List[int]:
    ...
    """
    working_state = twenty_rounds(state)
...
def serialize(state: List[int]) -> int:
    ...
    for word in state:
        result_bytes.extend(word.to_bytes(4, 'little'))
...
def next_state(state: List[int]) -> List[int]:
    ...
    new_state[12] = (new_state[12] + 1) % 4294967296
    return new_state
